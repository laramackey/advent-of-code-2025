...
bool is_accessible(const std::vector<std::vector<int>> &grid, int x, int y, int x_length, int y_length)
{
    int count = 0;
    for (int delta_x = -1; delta_x <= 1; ++delta_x)
    {
        for (int delta_y = -1; delta_y <= 1; ++delta_y)
        {
            ...
            int new_x = x + delta_x;
            int new_y = y + delta_y;
            if (new_y >= 0 && new_y < y_length &&
                new_x >= 0 && new_x < x_length)
            {
                if (grid[new_y][new_x] == 1 && ++count == 4) return false;
            }
        }
    }
    return true;
...
Result find_paper_rolls(std::vector<std::vector<int>> &grid, int task)
{
    ...
    for (int y = 0; y < y_length; ++y)
    {
        ...
        {
            if (grid[y][x] == 1 && is_accessible(grid, x, y, x_length, y_length))
            {
                accessible_count++;
